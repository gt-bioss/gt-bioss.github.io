...
*Amphithéâtre Hélène Martel-Massignac (anciennement amphi BDD), Pavillon Curie, 11 rue Pierre et Marie Curie, Paris* - [Carte](https://www.openstreetmap.org/way/56066140)
  ...
  - 15h45 - 17h15: *Contributed talks*:
    ...
    - 16h00: **Madeleine Eyraud** (CNRS, Université de Lille)
...
Le fonctionnement des écosystèmes marins dépend des interactions entre espèces, qui influencent des services essentiels comme la régulation du climat ou la qualité de l’eau. Comprendre ces interactions chez le phytoplancton, qui est à la base des réseaux trophiques («chaînes alimentaires»), est crucial face au changement global. Si les effets de l’environnement non vivant sont bien quantifiés , il a été montré dans de précédents travaux que les interactions entre espèces (compétition, allélopathie) sont aussi déterminantes mais mal comprises. Un premier travail d’apprentissage sur des données d’observation, à l’aide de méthodes classiques de machine learning, met en évidence l’importance de ces interactions entre espèces. Un second objectif est de caractériser ces interactions à l’aide d’une méthode d’apprentissage automatique explicable (LFIT), qui génère un programme logique à partir de données d’observation. Cette méthode nécessitant une représentation discrète des données, une approche de discrétisation différenciée par espèce s’appuyant sur des connaissances a priori a été choisie, et reste en cours d’évaluation. Les règles logiques issues du programme construit permettent d’extraire des connaissances, notamment sous forme de graphe d’influences entre espèces. La construction de ce graphe se base sur le nombre de correspondances des conditions d’une règle avec le jeu de données, ainsi que la probabilité que sa conclusion se réalise à l’instant suivant. Les influences probables entre espèces identifiées sont représentées sous forme de liens directionnels pondérés dans le graphe. Ces liens permettent de hiérarchiser l’importance des interactions entre espèces dans leur développement, et de les caractériser.
...
### Karim Raqbi (Université de Lille)
